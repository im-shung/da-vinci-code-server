...
//            }
        ...
        public void WriteChatMsg(ChatMsg obj) {
            try {
                String s = String.format("To 【%s】 code = %s ┃ data = %s\n", obj.UserName, obj.code, obj.data);
                System.out.print(s);
        ...
        public void WriteChatList(ChatMsg obj) {
            try {
                String s = String.format("To 【%s】 code = %s ┃ data = %s ┃ list = %s\n", obj.UserName, obj.code, obj.data,obj.list);
                System.out.print(s);
                oos.writeObject(obj.code);
                oos.writeObject(obj.UserName);
                oos.writeObject(obj.data);
        ...
        public void run() {
            while (true) { // 사용자 접속을 계속해서 받기 위해 while문
                ...
                if (cm.code.matches("MATCHCARD")) {
                    ...
                    // 고려점
                    // 1. 카드가 다 공개되었는가 -> GAMEOVER || LOOSE
                    if (ownerCardIndex != null) {
                        AppendText("SUCCESS!!!");

                        if (cardManager.isSameCardOpens(cardOwner)) { // 카드를 맞춰서 cardOwner의 모든 카드가 공개된 경우
                            cardOpenWithCondition(cardManager,room,cardOwner); // GAMEOVER || LOOSE 정해짐
                        }
                        else { // 아니면 맞춘 카드 방송
                    ...
                    // 고려점
                    // 1. 카드가 다 공개되었는가 -> GAMEOVER || LOOSE
                    // 2. 새로 뽑은 카드가 있는가 -> 없으면 CARDSELECT로 공개할 인덱스 받아야 함
                    else {
                        AppendText("FAIL!!!");

                        if (cardManager.isSameCardOpens(UserName)) { // 카드를 맞춰서 cardOwner의 모든 카드가 공개된 경우
                            cardOpenWithCondition(cardManager,room,UserName); // GAMEOVER || LOOSE 정해짐
                        }
                        else {
                            ChatMsg obcm = new ChatMsg(UserName, "FAIL", "match card fail!");
                            WriteChatMsg(obcm);

                            if (cardManager.RoomCards.size() == 0) { // 새로 뽑은 카드가 없을 때
    ...
    abstract class Subject {
        ...
        public String takeCard(String owner, String color) {
            ...
            System.out.println(owner+"이 뽑은 카드 정보: "+c.cardColor + c.cardNum+", 인덱스: "+o.newCardIndex);

            System.out.println(owner+"의 카드 리스트 업데이트");
            o.print();
